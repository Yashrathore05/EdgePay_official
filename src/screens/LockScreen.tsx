...
import { authenticate, isBiometricAvailable, hashPin } from '../engine/BiometricService';
import { useStore } from '../store/useStore';
import { useTheme, spacing, borderRadius, typography } from '../theme';
import { PinScreen, PinScreenRef } from '../components/PinScreen';
...
export const LockScreen: React.FC = () => {
  ...
  const onPinVerified = (pin: string) => {
    if (hashPin(pin) === pinHash) {
      ...
      pinScreenRef.current?.triggerError('Invalid PIN');
    }
  };
      ...
      <Animated.View style={[s.content, { opacity: fadeAnim }]}>
        ...
        {!!pinHash && (
          <TouchableOpacity 
            ...
            activeOpacity={0.7}
          >
            <Text style={{ color: colors.primary, fontWeight: '700' }}>Use PIN instead</Text>
          </TouchableOpacity>
        )}

        <Text style={[s.hint, { color: colors.textTertiary, marginTop: 20 }]}>Securely encrypted for your safety</Text>
      </Animated.View>
