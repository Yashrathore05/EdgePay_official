/**
 * SHA-256 helper for cryptographic PIN hashing.
 * Dependency-free, pure TypeScript implementation.
 */
...
function sha256(ascii: string): string {
  ...
  for (let chunkStart = 0; chunkStart < words.length; chunkStart += 16) {
    const w = words.slice(chunkStart, chunkStart + 16);
...
export function hashPin(pin: string): string {
  if (!pin) return '';
  // Salt with custom static string to make rainbow table attack harder
  const salt = 'EdgePay_Secret_Salt_2026';
